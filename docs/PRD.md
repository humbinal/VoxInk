...
- 📝 **M7 落地**：已注册 `aliyun_bailian_offline`/`aliyun_bailian_filetrans`/`aliyun_bailian_streaming`/`generic_ws`。应用层用 `resolve_backend_id(config, want_streaming, audio_len)` 按**配置 backend_id + 能力**选后端：配置项支持该模式则用之（百炼离线大文件透明转 filetrans），否则用合理默认。应用层仅依赖 trait + 注册表，新增后端无需改核心（开闭原则）。
